...
export const AvatarAnimation: React.FC<AvatarAnimationProps> = ({
  ...
  // Update avatar image based on player state
  useEffect(() => {
    switch (playerState) {
      case 'waiting':
        setAvatarImage('/avatar-waiting.svg');
        break;
      case 'riding':
  ...
  return (
    <div className="avatar-animation-container" style={{ width: chartWidth, height: chartHeight }}>
      {/* Dragon silhouette overlay - Toruk Makto background */}
      <motion.div 
      ...
      {playerState === 'riding' && (
        <motion.div
          ...
          style={{
            backgroundImage: `url('/IMAGE/ride.png')`,
            opacity: 0.7,
            width: '100%',
            height: '100%',
            position: 'absolute',
